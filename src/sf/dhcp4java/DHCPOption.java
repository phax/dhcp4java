...
//    }
    ...
    public byte getValueAsByte() throws IllegalArgumentException {
        ...
        if (value.length != 1)
        	throw new DHCPBadPacketException("option "+code+" is wrong size:"+value.length+" should be 1");
        return value[0];
    }
    /**
    ...
    public short getValueAsShort() throws IllegalArgumentException {
        ...
        if (value == null)
        	throw new IllegalStateException("value is null");
        if (value.length != 2)
        	throw new DHCPBadPacketException("option "+code+" is wrong size:"+value.length+" should be 2");
    ...
    public int getValueAsInt() throws IllegalArgumentException {
        ...
        if (value == null)
        	throw new IllegalStateException("value is null");
        if (value.length != 4)
        	throw new DHCPBadPacketException("option "+code+" is wrong size:"+value.length+" should be 4");
    ...
    public short[] getValueAsShorts() throws IllegalArgumentException {
        ...
        if (value == null)
        	throw new IllegalStateException("value is null");
        if ((value.length % 2) != 0)		// multiple of 2
        	throw new DHCPBadPacketException("option "+code+" is wrong size:"+value.length+" should be 2*X");
    ...
    public InetAddress[] getValueAsInetAddrs() throws IllegalArgumentException {
        ...
        if (value == null)
        	throw new IllegalStateException("value is null");
        if ((value.length % 4) != 0)		// multiple of 4
            throw new DHCPBadPacketException("option "+code+" is wrong size:"+value.length+" should be 4*X");
        try {
     ...
     */
    static private final int unsignedByte(byte b) {
        return (b & 0xFF);
    }
    ...
    public static Map<Byte, String> agentOptionsToMap(byte[] buf) {
        ...
        int i=0;
        while (i<buf.length) {
            if (buf.length - i < 2) break;	// not enough data left
            Byte key = buf[i++];
            int size = unsignedByte(buf[i++]);
            int instock = buf.length - i;
            if (size > instock) size = instock;
    ...
    //
    private final static Object[] _OPTION_FORMATS = {
